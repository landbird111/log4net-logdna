...
{
    internal class LogdnaFormatter : ILogdnaFormatter
    {
        ...
        public LogdnaFormatter(Config config)
        {
            _config = config;
            ...
            if (eventSize > _config.MaxEventSizeBytes)
            {
                ...
                if (bytesOver > 0)
                {
                    ErrorReporter.ReportError(
                        $"LogdnaFormatter: Dropping log event exceeding allowed limit of {_config.MaxEventSizeBytes} bytes. " +
